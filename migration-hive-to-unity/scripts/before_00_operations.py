...
def calcular_datas(bronzePath):
    ...
    try:
        # Leitura da bronze
        df_bronze_calculo = spark.read.format("delta").load(bronzePath)
...
def calcular_datas_silver(DfCalc, NomeTabela):
    if spark.catalog.tableExists(f"db_auth_silver.tb_{NomeTabela}"):
        # Leitura da tabela final
        max_silver = spark.sql(f"select max(ts_event) as max_date from db_auth_silver.tb_{NomeTabela}").collect()[0][0]
...
def transformation_raw_to_bronze_auth(df: DataFrame):
  return(
    ...
    .select(
      "*"
      ,input_file_name().alias('file_name_raw')
