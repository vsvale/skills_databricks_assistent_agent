...
# MAGIC %md
# MAGIC ### Ingestão da base Login e Signup Auth Back-End - RawDLH to Bronze
# MAGIC # 
...
# MAGIC
# MAGIC   - s3://serasaexperian-ecs-datalakehouse-prd-bronze/ecs/auth/login/back
# MAGIC   - s3://serasaexperian-ecs-datalakehouse-prd-bronze/ecs/auth/signup/back
# MAGIC
# MAGIC
# MAGIC
...
namedBronzeStream      = f"ecs_auth_back_{source}_write_raw_to_bronze"
...
if reprocess == "true":
    dbutils.fs.rm(f"/Volumes/auth_prd/bronze/checkpoints/ecs-datalakehouse-prd-checkpoints/ecs/auth/{source}/back")
...
for i in range(len(start_dates)):
  ...
  rawDF = rawDF.withColumn("attributes", from_json("attributes", attributes_back_schema))
  print('schema aplicado na coluna attributes')

  if source == "login":
    rawDF = rawDF.filter(col("attributes.context.flow").isin("sign_in", "LOGIN"))
    print('dados login filtrado')
  elif source == "signup":
    rawDF = rawDF.filter(col("attributes.context.flow").isin("signup", "SIGNUP"))
  ...
  try:
      ...
      bronzeWriter.toTable(bronzePath).awaitTermination()
